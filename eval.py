...
def parse_args():
    ...
    parser.add_argument('--input_h', type=int, default=1024, help='input height')#512
    parser.add_argument('--input_w', type=int, default=1024, help='input width')#512
    ...
    parser.add_argument('--eval_type', type=str, default='bbox', help='seg or dec')
...
class InstanceHeat(object):
    ...
    def load_weights(self, resume, args):
        print('loading weights from {}'.format(os.path.join(args.postrained_weights, resume)))
...
def run_seg_ap(object_is, args):
    ...
    iou_list = []
    for v in thresh:
        ap, iou = object_is.instance_segmentation_evaluation(args, ov_thresh=v, use_07_metric=True)
        ap_list.append(ap*100)
        iou_list.append(iou*100)
    print('seg_ap_list.txt', ap_list)
    print('seg_iou_list.txt', iou_list)
    np.savetxt('seg_ap_list.txt', ap_list, fmt='%.4f')
    np.savetxt('seg_iou_list.txt', iou_list, fmt='%.4f')
...
def run_dec_ap(object_is, args):
    ...
    ap_list = []
    for v in thresh:
        ap = object_is.detection_evaluation(args, ov_thresh=v, use_07_metric=True)
        ap_list.append(ap*100)
    print('dec_ap_list.txt', ap_list)
    np.savetxt('dec_ap_list.txt', ap_list, fmt='%.4f')
